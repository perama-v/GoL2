...
##### Description #####
#
# This is a module used by GoL2_regenerating. It is responsible
# for calculating a single evolution and passing that back to the
# game contract. The aim is to reduce complexity in that contract.
#
#######################
...
func evolve{}(
        ...
        cell_states_init_len : felt
        cell_states_init : felt*
    ) -> (
        cell_states_len : felt
        cell_states : felt*
    ):
    # Accepts an array representing the flattened game state.
    # Returns an array of same shape after evolving.
    let (local cell_states : felt*) = evaluate_rounds(
        number_of_generations, cell_states_init)
    return (cell_states, cell_states_len)
end
