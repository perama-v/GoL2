...
async def game_factory():
    ...
    # Deploy
    game = await starknet.deploy("contracts/GoL2.cairo")
    account = await starknet.deploy("contracts/Account.cairo")
    token = await starknet.deploy("contracts/ERC721.cairo")
    # Set up account
    ...
    spawn_game = signer.build_transaction(
        account, game.contract_address, 'spawn', [token.contract_address], 0)
    print('done')
    await spawn_game.invoke()
    return starknet, game, account, token


@pytest.mark.asyncio
async def test_game_flow(game_factory):
    # Start with freshly spawned game
    _, game, _, _ = game_factory
